...
CALCULATED_CHANNELS = {
    ...
    "FPRodHeave_Shape": lambda df: (
        (1 / 5) * (df["FPushrodFL"] + df["FPushrodFR"]) + (4 / 5) * (df["FPushrodRL"] + df["FPushrodRR"])
    ),
    "FPRodPitch": lambda df: (df["FPushrodFL"] + df["FPushrodFR"]) - (df["FPushrodRL"] + df["FPushrodRR"]),
    "FPRodPitch_Shape": lambda df: (
        (5 / 9) * (df["FPushrodFL"] + df["FPushrodFR"]) - (4 / 9) * (df["FPushrodRL"] + df["FPushrodRR"])
    ),
    ...
    "FProdVarFL": lambda df: abs(
        df["FPushrodFL"] - df["FPushrodFL"].rolling(window=32, min_periods=1, center=True).mean()
    ),
    "FProdVarFR": lambda df: abs(
        df["FPushrodFR"] - df["FPushrodFR"].rolling(window=32, min_periods=1, center=True).mean()
    ),
    "FProdVarRL": lambda df: abs(
        df["FPushrodRL"] - df["FPushrodRL"].rolling(window=32, min_periods=1, center=True).mean()
    ),
    "FProdVarRR": lambda df: abs(
        df["FPushrodRR"] - df["FPushrodRR"].rolling(window=32, min_periods=1, center=True).mean()
    ),
    ...
    "xDamperVarFL": lambda df: abs(
        df["xDamperFL"] - df["xDamperFL"].rolling(window=32, min_periods=1, center=True).mean()
    ),
    "xDamperVarFR": lambda df: abs(
        df["xDamperFR"] - df["xDamperFR"].rolling(window=32, min_periods=1, center=True).mean()
    ),
    "xDamperVarRL": lambda df: abs(
        df["xDamperRL"] - df["xDamperRL"].rolling(window=32, min_periods=1, center=True).mean()
    ),
    "xDamperVarRR": lambda df: abs(
        df["xDamperRR"] - df["xDamperRR"].rolling(window=32, min_periods=1, center=True).mean()
    ),
    ...
    "hRideF": calc_channel("hRideF", "hRideFL", "hRideFR")(
        lambda df: df["hRideF"] if "hRideF" in df.columns else (df["hRideFL"] + df["hRideFR"]) / 2
    ),
    "hRideR": calc_channel("hRideR", "hRideRL", "hRideRR")(
        lambda df: df["hRideR"] if "hRideR" in df.columns else (df["hRideRL"] + df["hRideRR"]) / 2
    ),
    ...
    "ratio_time_in_SM_100": lambda df: (
        cumulative_trapezoid((df["SM"] >= 0.999).astype(float), dx=0.01, initial=0)
        / (cumulative_trapezoid(np.ones_like(df["SM"]), dx=0.01, initial=0) + 1e-6)
    ),
    "ratio_time_in_SM_90": lambda df: (
        cumulative_trapezoid((df["SM"] >= 0.9).astype(float), dx=0.01, initial=0)
        / (cumulative_trapezoid(np.ones_like(df["SM"]), dx=0.01, initial=0) + 1e-6)
    ),
    "ratio_time_in_SM_80": lambda df: (
