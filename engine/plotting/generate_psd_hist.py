"""Module-level generator functions extracted from ``PsdHistMixin``.
...
from ._psd_lorentz import _fit_lorentz_peak, _lorentz_peak_model


LORENTZ_COMPACT_THRESHOLD = 5


def generate_psd_plots(plotter):
    plotter._ensure_preprocessed()
    plots = plotter._get_plot_group(2)
    if not plots:
        return
    plotter._lorentz_fit_records = []
    plot_iter = plots if plotter.verbose else _tqdm(plots, desc="PSD", unit="plot", leave=True)
    for plot_def in plot_iter:
        ...
        if plot_def.nperseg is not None:
            ...
            for run in plotter.runs:
                ...
                if df is None:
                    continue
                run_rate = plotter.run_sample_rates.get(
                    run["name"].lower(),
                    (plotter.FILTER_SAMPLE_RATE, "default"),
                )[0]
            ...
            fs_ref = min(sample_rates) if sample_rates else float(plotter.FILTER_SAMPLE_RATE)
            nperseg = (
                datafunctions.auto_nperseg(
                    n_min,
                    sample_rate=fs_ref,
                    min_averages_target=plotter.PSD_MIN_AVERAGES_TARGET,
                )
                if n_min >= 16
                else 512
            )
        ...
        filename = plotter._sanitize_plot_filename("psd", plot_name)
        figsize = plotter._resolve_plot_figsize(filename, plotter.psd_FIGSIZE)
        fig, ax = plt.subplots(figsize=figsize)
        ax.set_xlabel("Frequency (Hz)", fontweight="bold")
        primary_ch = channels_list[0]
        ax.set_ylabel(
            datafunctions.format_psd_ylabel(primary_ch, plotter.units_map),
            fontweight="bold",
        )
        run_groups = {}
        for run in plotter.runs:
        ...
        for group_name, group_runs in run_groups.items():
            loaded_runs = [r for r in group_runs if r["name"].lower() in plotter.run_data]
            if not loaded_runs:
                log.warning(
            ...
            rate_info = plotter.run_sample_rates.get(
            ...
            for ch_idx, ch in enumerate(channels_list):
                ...
                for run in loaded_runs:
                    ...
                    if df is None or ch not in df.columns:
                        hint = plotter._format_missing_channel_hint(run_name, ch)
                        log.warning(
                    ...
                    # the same frequency resolution (Î”f) regardless of
                    # native logging rate.
                    if _nperseg_ref_rate and _nperseg_ref_rate > 0:
                        run_rate = plotter.run_sample_rates.get(run_name, (plotter.FILTER_SAMPLE_RATE, "default"))[0]
                        run_nperseg = max(64, int(nperseg * run_rate / _nperseg_ref_rate))
                    else:
                        run_nperseg = nperseg
                    freq, power, n_segs = plotter._cached_psd_with_segments(
                        run_name,
                        ch,
                        run_nperseg,
                        gate_spec=gate_spec,
                ...
                lbl = f"{group_name.upper()} â€” {ch}" if multi else group_name.upper()
        ...
        if not plotted_any:
            ...
            plt.close(fig)
            continue
        has_x_limits, has_y_limits = plotter._apply_2d_axis_limits(
            ax,
            axis_limits,
        ...
        plotter._add_standard_legend(ax, loc="best")
            ...
            for f_at in freq_targets:
                ...
                if ann_items:
                    ...
                    for i in reversed(range(n_items)):
                        ...
                        ax.scatter([f_at], [p_at], color=color_e, s=50, zorder=10, edgecolors="white", linewidths=1.2)
        ...
        if remaining_lorentz:
            ...
            for _win_key, items in by_window.items():
                ...
                if len(items) >= plotter.LORENTZ_COMPACT_THRESHOLD:
                    ...
                    plotter._draw_lorentz_compact_box(ax, _win_key, items, psd_curves)
                    continue
    ...
    plotter._log_lorentz_fit_summary()


def _draw_lorentz_compact_box(plotter, ax, window, items, psd_curves):
...
def _record_lorentz_fit(
    plotter, plot, group, channel, f_lo, f_hi, f0_fit, zeta, sigma_zeta, r_squared, saturated, failed=False
):
    records = getattr(plotter, "_lorentz_fit_records", None)
    if records is None:
        records = []
        plotter._lorentz_fit_records = records
    records.append(
        {
            "plot": plot,
            "group": group,
...
def _log_lorentz_fit_summary(plotter):
    records = getattr(plotter, "_lorentz_fit_records", None)
    if not records:
        return
...
def generate_histogram_plots(plotter):
    plotter._ensure_preprocessed()
    plots = plotter._get_plot_group(3)
    if not plots:
        return
    plot_iter = plots if plotter.verbose else _tqdm(plots, desc="Histogram", unit="plot", leave=True)
    for plot_def in plot_iter:
        plot_name = plot_def.name
        channel = plot_def.channel
        axis_limits = plot_def.axis_limits
        ...
        filename = plotter._sanitize_plot_filename("histogram", plot_name)
        figsize = plotter._resolve_plot_figsize(filename, plotter.histogram_FIGSIZE)
        fig, ax = plt.subplots(figsize=figsize)
        ax.set_xlabel(
        ...
        for run in plotter.runs:
            ...
            if data.empty:
                continue
            dt = 1.0 / plotter._run_fs(run_name)
        ...
        if len(bins) > 1:
            ...
            if len(bins) <= 31:
                ...
                ax.grid(True, which="minor", axis="x", **plotter.GRID_STYLE["minor"])
            ax.grid(True, which="major", axis="x", **plotter.GRID_STYLE["major"])
        plotter._add_axis_edge_padding(
            ax,
            x_pad_ratio=(0 if has_x_limits else 0.02),
