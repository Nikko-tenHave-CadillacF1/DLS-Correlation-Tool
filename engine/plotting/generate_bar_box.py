"""Module-level generator functions extracted from ``BarBoxMixin``.
...
def generate_bar_plots(plotter):
    plotter._ensure_preprocessed()
    plots = plotter._get_plot_group(4)
    if not plots:
        return
    plot_iter = plots if plotter.verbose else _tqdm(plots, desc="Bar", unit="plot", leave=True)
    for plot_def in plot_iter:
        ...
        filename = plotter._sanitize_plot_filename("bar", plot_name)
        figsize = plotter._resolve_plot_figsize(filename, plotter.bar_FIGSIZE)
        fig, ax = plt.subplots(figsize=figsize)
        x = np.arange(len(metric_specs))
        loaded_runs = [run for run in plotter.runs if run["name"].lower() in plotter.run_data]
        if not loaded_runs:
        ...
        for run_index, run in enumerate(loaded_runs):
            ...
            for channel, aggregation in metric_specs:
                if channel not in df.columns:
                    hint = plotter._format_missing_channel_hint(run_name, channel)
                    log.warning(
                ...
                values.append(
                    datafunctions.aggregate_channel_for_bar(
                        ...
                        sample_rate=plotter._run_fs(run_name),
        ...
        if getattr(plot_def, "secondary_axis", True) and len(all_values) > 1:
            ...
            candidate_threshold = max_abs / max(1.0, plotter.BAR_SECONDARY_AXIS_RATIO)
            ...
            if lower_group and max(lower_group) > 0:
                ...
                if ratio >= plotter.BAR_SECONDARY_AXIS_RATIO:
                    ax2 = ax.twinx()
                    ...
                    log.info(
                        ...
                        plot_name,
                        ratio,
                        plotter.BAR_SECONDARY_AXIS_RATIO,
                    )
        plotted_labels = set()
        bar_info = []
        ...
        if len(metric_specs) == 1:
            ...
            ax.set_ylabel(
                datafunctions.add_units_to_label(channel_name, plotter.units_map),
                fontweight="bold",
                fontsize=12,
            )
        ...
        plotter._add_axis_edge_padding(ax, x_pad_ratio=0.06, y_pad_ratio=0.04)
        if ax2 is not None:
            plotter._add_axis_edge_padding(ax2, x_pad_ratio=0.06, y_pad_ratio=0.04)
        ...
        plotter._apply_grid(ax, which="major", axis="y")
        ax.spines["top"].set_visible(False)
        ax.spines["right"].set_visible(False)
        if ax2 is not None:
            ax2.grid(True, axis="y", alpha=plotter.GRID_STYLE["minor"]["alpha"])
        ...
        for axis in [ax, ax2] if ax2 is not None else [ax]:
            for h, l in zip(*axis.get_legend_handles_labels()):
                if l and l != "_nolegend_" and l not in labels:
                    handles.append(h)
                    labels.append(l)
        plotter._add_standard_legend(ax, handles=handles, labels=labels, loc="upper right")
        plotter._draw_horizontal_reference_lines(ax, reference_lines)
        if gate_spec is not None:
            gate_text = datafunctions.format_gate_text(gate_spec)
            if gate_text:
                legend_obj = ax.get_legend()
                plotter._display_gate_info(ax, gate_text, legend=legend_obj)
...
def _parse_boxplot_definition(plotter, plot_def):
    plot_name = plot_def.name
...
def _collect_boxplot_point_series_from_data(plotter, channel, filtered_run_data):
    series = []
...
def _apply_boxplot_artist_style(plotter, bp, box_settings, colors=None, facecolor=None, alpha=0.82):
...
def generate_box_plots(plotter):
    plotter._ensure_preprocessed()
    plots = plotter._get_plot_group(5)
    if not plots:
        return
    box_settings = getattr(plotter, "BOX_PLOT_SETTINGS", {})
    plot_iter = plots if plotter.verbose else _tqdm(plots, desc="Box", unit="plot", leave=True)
    for plot_def in plot_iter:
        if getattr(plot_def, "kind", None) == "box_grid":
            plotter._generate_boxplot_grid(plot_def, box_settings)
            continue
        try:
            plot_name, channels, aggregation_mode, axis_limits, gate_spec, options = plotter._parse_boxplot_definition(
                plot_def
            )
        ...
        figsize = (
            box_settings.get("figsize_single_channel", plotter.boxplot_FIGSIZE)
            if len(channels) == 1
            else box_settings.get("figsize_multi_channel", plotter.boxplot_FIGSIZE)
        )
        if aggregation_mode == "per_run":
            plotter._generate_boxplot_per_run(plot_name, channels, axis_limits, gate_spec, plot_options, figsize)
        elif aggregation_mode == "aggregated":
            plotter._generate_boxplot_aggregated(plot_name, channels, axis_limits, gate_spec, plot_options, figsize)
        elif aggregation_mode == "per_run_aggregated":
            plotter._generate_boxplot_per_run_aggregated(
...
def _generate_boxplot_per_run(plotter, plot_name, channels, axis_limits, gate_spec, options, figsize):
    box_settings = options
    filtered_run_data = {rn: plotter._get_filtered_run_dataframe(rn, gate_spec) for rn in plotter.run_data}
    ...
    if not agg_data:
        ...
        return
    run_names = [r["name"].lower() for r in plotter.runs if r["name"].lower() in agg_data]
    if not run_names:
        return
    run_colors = {r["name"].lower(): r["color"] for r in plotter.runs}
    ...
    for ax, channel in zip(axes, channels):
        ...
        plotter._apply_boxplot_artist_style(
            bp,
            box_settings,
            colors=colors_list,
        ...
        ax.set_ylabel(
            datafunctions.add_units_to_label(channel, plotter.units_map),
            fontweight="bold",
            fontsize=12,
        )
        if isinstance(axis_limits, (list, tuple)) and len(axis_limits) == 2:
    ...
    if gate_text:
        plotter._display_gate_info(axes[0], gate_text)
    ref_lines_spec = options.get("_reference_lines") if isinstance(options, dict) else None
    if ref_lines_spec:
        for ax in axes:
            plotter._draw_horizontal_reference_lines(ax, ref_lines_spec)
    plot_title = box_settings.get("title")
    if plot_title:
        fig.suptitle(plot_title, fontsize=14, fontweight="bold", y=1.02)
    plt.tight_layout(pad=0.25)
    filename = plotter._sanitize_plot_filename("box", plot_name)
    fig.savefig(
        plotter.plots_dir / filename, dpi=plotter.output_dpi, pad_inches=0.15, facecolor="white", bbox_inches="tight"
    )
    plt.close(fig)
    if plotter.verbose:
        log.debug("Saved: %s", filename)
...
def _generate_boxplot_aggregated(plotter, plot_name, channels, axis_limits, gate_spec, options, figsize):
    box_settings = options
    filtered_run_data = {rn: plotter._get_filtered_run_dataframe(rn, gate_spec) for rn in plotter.run_data}
    agg_data = datafunctions.aggregate_channel_for_boxplot(
        plotter.run_data,
    ...
    for ax, channel in zip(axes, channels):
        ...
        plotter._apply_boxplot_artist_style(bp, box_settings, facecolor=agg_color, alpha=agg_alpha)
        ax.set_ylabel(
            datafunctions.add_units_to_label(channel, plotter.units_map),
            fontweight="bold",
            fontsize=12,
        ...
        if yl <= 0 <= yr:
            ax.axhline(0, color="#5E5E5E", linewidth=1, alpha=0.8)
        if show_points:
            overlay_series = plotter._collect_boxplot_point_series_from_data(channel, filtered_run_data)
    ...
    if show_points and legend_handles:
        plotter._add_standard_legend(axes[0], handles=legend_handles, labels=legend_labels, loc="upper right")
    if gate_text:
        legend_obj = axes[0].get_legend() if show_points and legend_handles else None
        plotter._display_gate_info(axes[0], gate_text, legend=legend_obj)
    ref_lines_spec = options.get("_reference_lines") if isinstance(options, dict) else None
    if ref_lines_spec:
        for ax in axes:
            plotter._draw_horizontal_reference_lines(ax, ref_lines_spec)
...
def _generate_boxplot_per_run_aggregated(plotter, plot_name, channels, axis_limits, gate_spec, options, figsize):
    box_settings = options
    filtered_run_data = {rn: plotter._get_filtered_run_dataframe(rn, gate_spec) for rn in plotter.run_data}
    ...
    if not per_run_data and not agg_data:
        log.warning("Box plot '%s': no data after aggregation.", plot_name)
        return
    run_names = [r["name"].lower() for r in plotter.runs if r["name"].lower() in per_run_data]
    run_colors = {r["name"].lower(): r["color"] for r in plotter.runs}
    show_points = bool(box_settings.get("show_points", False))
    show_fliers = bool(box_settings.get("show_fliers", True))
    point_alpha = float(box_settings.get("point_alpha", 0.25))
...
def _generate_boxplot_grid(plotter, grid_def, box_settings):
    ...
    for channel in channels:
        ...
        for r_idx, row_label in enumerate(row_labels):
            ...
            for c_idx, col_label in enumerate(col_labels):
                ...
                if aggregation_mode == "aggregated":
                    plotter._render_grid_cell_aggregated(
                        ax,
                        channel,
                        combined_gate,
                        options,
                    ...
                    plotter._render_grid_cell_per_run(
                        ax,
                        channel,
                        combined_gate,
                        options,
        ...
        channel_label = datafunctions.add_units_to_label(channel, plotter.units_map)
        suptitle = f"{plot_name} — {channel_label}"
        if event_title:
            suptitle = f"{event_title} | {suptitle}"
        fig.suptitle(suptitle, fontsize=13, fontweight="bold", y=1.02)
        plt.tight_layout(pad=0.4)
        grid_filename = plotter._sanitize_plot_filename("box_grid", f"{plot_name}_{channel}")
        fig.savefig(
            plotter.plots_dir / grid_filename,
            dpi=plotter.output_dpi,
            pad_inches=0.15,
            facecolor="white",
            bbox_inches="tight",
...
def _render_grid_cell_aggregated(
    ...
    if len(data) == 0:
        ...
        ax.set_xticks([])
        return
    bp = ax.boxplot([data], labels=[channel], patch_artist=True, widths=box_width, showfliers=show_fliers, whis=[5, 95])
    plotter._apply_boxplot_artist_style(bp, options, facecolor=agg_color, alpha=agg_alpha)
    ax.set_xticklabels([])
    if show_points:
        overlay_series = plotter._collect_boxplot_point_series_from_data(channel, filtered_run_data)
...
def _render_grid_cell_per_run(
    ...
    plotter._apply_boxplot_artist_style(bp, options, colors=colors_list, alpha=0.7)
