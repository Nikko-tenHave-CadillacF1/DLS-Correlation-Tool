...
from .workflow.color_utils import (  # noqa: F401  (back-compat re-export)
...
def _resolve_consolidate_by(spec):
    ...
    if isinstance(spec, str):
        ...
        try:
            ...
            raise ValueError(f"consolidate_by={spec!r} is not a valid regex: {exc}") from exc
        if pattern.groups < 1:
            raise ValueError(
                f"consolidate_by regex {spec!r} must contain at least one capture group identifying the partition key."
            )
        ...
        return _extract
    raise TypeError(
        f"consolidate_by must be None, a preset name, a regex string, or a callable; got {type(spec).__name__}."
    )
    ...
    for i, run in enumerate(runs):
        if "folder" not in run:
            if run.get("split_by") and (
                run.get("consolidate") or run.get("consolidate_by") or "_consolidate_sources" in run
            ):
                raise ValueError(
        ...
        if not filetype:
            raise ValueError(
                f"Folder run[{i}] {folder_rel!r}: missing 'filetype' (one of {sorted(_VALID_RUN_FILETYPES)})."
            )
        ext = filetype.lower()
        ...
        if ext not in _VALID_RUN_FILETYPES:
            raise ValueError(
                f"Folder run[{i}] {folder_rel!r}: filetype {filetype!r} must be one of {sorted(_VALID_RUN_FILETYPES)}."
            )
        run_type = run.get("type")
        ...
        if not folder_path.is_dir():
            raise FileNotFoundError(f"Folder run[{i}]: directory not found -> {folder_path}")
        files = sorted(p for p in folder_path.iterdir() if p.is_file() and p.suffix.lower() == ext)
        if not files:
            raise FileNotFoundError(f"Folder run[{i}] {folder_rel!r}: no '{ext}' files in {folder_path}.")
        contains = run.get("contains")
        if contains:
            if not isinstance(contains, str):
                raise ValueError(
                    f"Folder run[{i}] {folder_rel!r}: 'contains' must be a string, got {type(contains).__name__}."
                )
            needle = contains.lower()
            files = [p for p in files if needle in p.name.lower()]
            if not files:
                raise FileNotFoundError(
                    f"Folder run[{i}] {folder_rel!r}: no '{ext}' files in {folder_path} matched contains={contains!r}."
                )
        ...
        if consolidate not in (None, *_VALID_CONSOLIDATE_MODES):
            raise ValueError(
                f"Folder run[{i}] {folder_rel!r}: consolidate must be True or 'only'; got {consolidate!r}."
            )
        ...
        try:
            ...
            raise ValueError(f"Folder run[{i}] {folder_rel!r}: {exc}") from exc
        produced: list = []
        ...
        for j, f in enumerate(files):
            ...
            if colors:
                ...
                entry["color"] = _FOLDER_RUN_COLOR_PALETTE[j % len(_FOLDER_RUN_COLOR_PALETTE)]
        ...
        if consolidate in (True, "only"):
            if partition_fn is None:
                ...
                if not groups:
                    raise ValueError(
                        f"Folder run[{i}] {folder_rel!r}: consolidate_by produced no groups (all files returned None)."
                    )
                if consolidate == "only":
                    expanded.extend(produced)
...
def validate_config(config: PlotJobConfig) -> list[str]:
    ...
    for i, run in enumerate(config.runs):
        ...
        if is_consolidated:
            ...
            if not sources:
                issues.append(f"Run '{label}': consolidated entry has empty _consolidate_sources.")
        ...
        if run_type and run_type not in _VALID_RUN_TYPES:
            issues.append(
                f"Run '{label}': unknown type '{run_type}'. Expected one of: {', '.join(sorted(_VALID_RUN_TYPES))}"
            )
...
def validate_export_map(plot_definitions: tuple, export_map: dict | None) -> list[str]:
    ...
    warnings = []
    if orphans:
        warnings.append(f"[WARNING] {len(orphans)} plot(s) generated but NOT in POWERPOINT_EXPORT_MAP:")
    ...
    for i, group in enumerate(config.plot_definitions or []):
        ...
        for plot_def in group:
            ...
            for run in runs:
                ...
                if missing:
                    missing_per_run.append(
                        f"{run['name']}: {', '.join(missing[:5])}{' ...' if len(missing) > 5 else ''}"
                    )
        ...
        with ZipFile(template_path) as pptx:
            ...
            for slide_num, config in export_map.items():
                ...
                for i, img_file in enumerate(image_files):
                    ...
                    if config.get("layout") == "main_plot" and slide_width is not None and len(image_files) == 1:
                        w = slide_width
                    slide_aspects.append((img_file, w / h))
                if (
                    config.get("layout") == "double_plot"
                    and len(slide_aspects) == 2
                    and not all(name.startswith(("scatter_", "psd_", "bar_")) for name, _ in slide_aspects)
                ):
...
def _export_via_pptx(template_path, output_path, plots_dir, export_map):
    ...
    def _iter_pic_elements(slide):
        ...
        return list(sp_tree.iter("{http://schemas.openxmlformats.org/drawingml/2006/main}pic")) + list(
            sp_tree.iter("{http://schemas.openxmlformats.org/presentationml/2006/main}pic")
        )
    ...
    for slide_num, config in export_map.items():
        ...
        if idx < 0 or idx >= len(prs.slides):
            log.warning("Slide %s out of range (template has %d slides). Skipping.", slide_num, len(prs.slides))
            continue
